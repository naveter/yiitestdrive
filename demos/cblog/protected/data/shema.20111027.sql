...
--
-- Хост: localhost
-- Время создания: Окт 27 2011 г., 18:43
...
(2, 'Вы, возможно, заметили, что каркас приложения уже реализует аутентификацию, проверяя, являются ли имя пользователя и пароль значениями demo и admin. В этом разделе мы изменим соответствующий код так, чтобы аутентификация происходила посредством таблицы User БД.', 2, 1230953187, 'Thomas Gates', 'gates@gmail.sru', 'http://mail.ru/index.php?foo=bar', 1);
...
(3, 'Пост, созданный Демой`', 'Подсказка: В соответствии с соглашением, название файла класса должно быть таким же как имя класса плюс расширение .php. Можно обратиться к классу, используя псевдоним пути. Например, мы можем обратиться к классу UserIdentity используя псевдоним application.components.UserIdentity. Многие методы API в Yii могут распознать псевдонимы пути (например, Yii::createComponent()). Это позволяет избежать использования абсолютных путей в коде, которые создают неприятности при развертывании приложения.', 'yii, rts', 2, 1230953187, 1230953187, 1);
